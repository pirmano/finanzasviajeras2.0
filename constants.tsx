...
const IconWrapper: React.FC<{ children: React.ReactNode, className?: string }> = ({ children, className = "w-5 h-5" }) => {
  return React.isValidElement(children) ? React.cloneElement(children as React.ReactElement<any>, { className }) : null;
};
...
Object.values(ExpenseCategory).forEach(category => {
  if (CATEGORY_DETAILS[category]) {
    CATEGORY_DETAILS[category].icon = <IconWrapper>{CATEGORY_DETAILS[category].icon}</IconWrapper>;
  }
});
...
export const INFO_ITEM_TYPE_DETAILS: Record<InfoItemType, { icon: React.ReactNode; color: string }> = {
  [InfoItemType.BOARDING_PASS]: {
    icon: <IconWrapper><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" /></svg></IconWrapper>,
    color: 'bg-blue-500',
  },
